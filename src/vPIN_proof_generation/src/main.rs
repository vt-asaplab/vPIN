...
fn main() {
  ...
  let (
    ...
    println!("Proof generation time: {:?} ms", SystemTime::now().duration_since(sy_time).unwrap().as_millis());
    ...
    println!("Proof verification time: {:?} ms", SystemTime::now().duration_since(sy_time2).unwrap().as_millis());
